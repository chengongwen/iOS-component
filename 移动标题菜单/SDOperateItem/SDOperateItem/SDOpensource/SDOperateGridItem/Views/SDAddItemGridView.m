...
{
    if ([SDGridItemCacheTool addItemsList:view.itemModel.title]) {
        ...
        UIAlertView *alter = [[UIAlertView alloc] initWithTitle:nil message:@"添加成功" delegate:nil cancelButtonTitle:@"ok" otherButtonTitles: nil];
        [alter show];
    }
    else {
        
        UIAlertView *alter = [[UIAlertView alloc] initWithTitle:nil message:@"首页标题栏已满，添加失败" delegate:nil cancelButtonTitle:@"ok" otherButtonTitles: nil];
        [alter show];
    }
}
...
- (void)saveItemsSettingCache
{
    ...
    // 保存当前首页标题数组列表
    [SDGridItemCacheTool saveAddItemsArrayList:[tempItemsContainer copy]];
