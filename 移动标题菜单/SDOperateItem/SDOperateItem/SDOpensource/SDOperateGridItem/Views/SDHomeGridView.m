...
- (void)setGridModelsArray:(NSArray *)gridModelsArray
{
    ...
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[SDHomeGridViewListItemView class]]) {
...
{
    if ([SDGridItemCacheTool removeItemsList:view.itemModel.title]) {
        ...
        UIAlertView *alter = [[UIAlertView alloc] initWithTitle:nil message:@"删除失败" delegate:nil cancelButtonTitle:@"ok" otherButtonTitles: nil];
        [alter show];
    }
}
...
- (void)saveItemsSettingCache
{
    ...
    // 保存当前首页标题数组列表
    [SDGridItemCacheTool saveHomeItemsArrayList:[tempItemsContainer copy]];
...
- (void)layoutSubviews
{
    ...
    _cycleScrollADView.frame = CGRectMake(0, _cycleScrollADViewBackgroundView.sd_y + 5, self.sd_width, itemH - 5 * 2);
}
