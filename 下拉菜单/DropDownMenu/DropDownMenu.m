...
+ (instancetype)indexPathWithCol:(NSInteger)col row:(NSInteger)row {
    DropDownIndexPath *indexPath = [[self alloc] initWithColumn:col row:row];
    return indexPath;
}
...
@implementation DropDownBackgroundCellView
...
- (NSString *)titleForRowAtIndexPath:(DropDownIndexPath *)indexPath {
...
- (void)selectDefalutIndexPath
{
    if (_dataSource && _delegate
        ...
        if (!_isClickHaveItemValid &&_dataSourceFlags.numberOfItemsInRow
            ...
            [_delegate menu:self didSelectRowAtIndexPath:[DropDownIndexPath indexPathWithCol:0 row:0 item:0]];
        } else if (_dataSourceFlags.numberOfRowsInColumn
                   && [_dataSource menu:self numberOfRowsInColumn:0] > 0){
            [_delegate menu:self didSelectRowAtIndexPath:[DropDownIndexPath indexPathWithCol:0 row:0]];
        }
    }
...
- (void)setDataSource:(id<DropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        if (!self.isClickHaveItemValid && _dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:0 column:i]>0) {
            titleString = [_dataSource menu:self titleForItemsInRowAtIndexPath:[DropDownIndexPath indexPathWithCol:i row:0 item:0]];
        }else {
            titleString =[_dataSource menu:self titleForRowAtIndexPath:[DropDownIndexPath indexPathWithCol:i row:0]];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        DropDownBackgroundCellView *bg = [[DropDownBackgroundCellView alloc]init];
    ...
    if (tableView == _leftTableView) {
        if (_dataSourceFlags.titleForRowAtIndexPath) {
            cell.textLabel.text = [_dataSource menu:self titleForRowAtIndexPath:[DropDownIndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
        ...
        if (_dataSourceFlags.titleForItemsInRowAtIndexPath) {
            cell.textLabel.text = [_dataSource menu:self titleForItemsInRowAtIndexPath:[DropDownIndexPath indexPathWithCol:_currentSelectedMenudIndex row:_currentSelectedMenudRow item:indexPath.row]];
        } else {
            NSAssert(0 == 1, @"dataSource method needs to be implemented");
        }
        if ([cell.textLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_leftTableView == tableView) {
        ...
        if (isClickHaveItemValid && _delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [self.delegate menu:self didSelectRowAtIndexPath:[DropDownIndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
        } else {
        ...
        if (self.delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [self.delegate menu:self didSelectRowAtIndexPath:[DropDownIndexPath indexPathWithCol:_currentSelectedMenudIndex row:_currentSelectedMenudRow item:indexPath.row]];
        } else {
            //TODO: delegate is nil
...
- (BOOL )confiMenuWithSelectRow:(NSInteger)row {
    ...
    if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:_currentSelectedMenudRow column:_currentSelectedMenudIndex]> 0) {
        ...
        if (self.isClickHaveItemValid) {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[DropDownIndexPath indexPathWithCol:_currentSelectedMenudIndex row:row]];
...
- (void)confiMenuWithSelectItem:(NSInteger)item {
    ...
    CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
    title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[DropDownIndexPath indexPathWithCol:_currentSelectedMenudIndex row:_currentSelectedMenudRow item:item]];
    [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
        _show = NO;
    }];
