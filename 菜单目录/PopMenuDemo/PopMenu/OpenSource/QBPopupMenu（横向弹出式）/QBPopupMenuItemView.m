...
- (void)performAction // 设置target 事件action 和 指定对象self.item
{
    if (self.item.target && self.item.action) {
        
//        [self.item.target performSelector:self.item.action withObject:self.item afterDelay:0];
        
        [self.item.target performSelectorOnMainThread:self.item.action withObject:self.item waitUntilDone:YES];
    }
